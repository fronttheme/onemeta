...
class UserFieldManager {
  ...
  private function render_user_field( int $user_id, string $group_id, string $field_id, array $field ): void {
    ...
    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Renderer::render() returns pre-escaped HTML; every dynamic value is escaped inside each type method.
  ...
  private function verify_user_nonce( string $group_id ): bool {
    ...
    return (bool) wp_verify_nonce(
        sanitize_text_field( wp_unslash( $_POST[ 'onemeta_user_' . $group_id . '_nonce' ] ?? '' ) ),
        'onemeta_user_' . $group_id
    );
  }
  ...
  private function save_user_group_fields( int $user_id, string $group_id, array $group ): void {
    // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified above via verify_user_nonce(); every value is sanitized per-type inside save_user_group_fields()/sanitize_by_type().
    $data = wp_unslash( $_POST['onemeta_user'][ $group_id ] ?? [] );
    ...
    foreach ( $group['fields'] as $field_id => $field ) {
      ...
      // Handle repeater fields separately
      if ( $type === 'repeater' ) {
        // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Same nonce check above; sanitized per-type inside sanitize_repeater_data()/sanitize_by_type().
        $repeater_data   = wp_unslash( $_POST['onemeta_user_repeater'][ $group_id ][ $field_id ] ?? [] );
