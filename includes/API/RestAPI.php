...
class RestAPI {
	...
	public function get_field_groups() {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,PluginCheck.Security.DirectDB.UnescapedDBParameter -- Custom plugin table; %i placeholder in $wpdb->prepare() already neutralizes $table_name.
	...
	public function create_field_group( $request ) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom plugin table; no core WP API exists for this query.
		$exists = $wpdb->get_var(
			$wpdb->prepare( 'SELECT COUNT(*) FROM %i WHERE group_key = %s', $table_name, $data['group_key'] )
		);
