...
class AdminAssets {
	...
	public function enqueue_admin_assets( string $hook ): void {
		...
		$this->add_admin_chrome_assets( $is_dev );
	...
	/**
	 * Inline admin-chrome CSS/JS — hides foreign notices on OneMeta pages and
	 * clears the default admin footer. Attached to the already-enqueued
	 * 'onemeta-builder-app' handle rather than printed as raw tags. CSS is
	 * skipped in dev mode since no style handle exists there (Vite injects
	 * CSS via the dev-server JS instead).
	 */
	private function add_admin_chrome_assets( bool $is_dev ): void {
		if ( ! $is_dev ) {
			$css = "
		...
		";
			...
			wp_add_inline_style( 'onemeta-builder-app', $css );
		}
	...
	";
		...
		wp_add_inline_script( 'onemeta-builder-app', $js );
	}
	...
	private function enqueue_dev_assets(): void {
		...
		wp_enqueue_script(
			...
			[],
			null, // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion -- Version intentionally omitted in dev mode; Vite's HMR handles cache invalidation.
			false
		);
	...
	private function enqueue_production_assets(): void {
		wp_enqueue_style(
			'onemeta-builder-app',
			ONEMETA_PLUGIN_URL . 'dist/css/admin.css',
			[],
			ONEMETA_VERSION
		);
		...
		wp_enqueue_script(
			'onemeta-builder-app',
			ONEMETA_PLUGIN_URL . 'dist/js/admin.min.js',
			[ 'jquery', 'wp-i18n' ],
			ONEMETA_VERSION,
			true
		);
	...
	private function enqueue_builder_data( string $page ): void {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading admin page parameter to preload builder data; no state change occurs here.
		$group_id         = isset( $_GET['id'] ) ? sanitize_text_field( wp_unslash( $_GET['id'] ) ) : '';
	...
	public function clean_admin_page(): void {
		...
		if ( ! $screen || ! str_contains( $screen->id, 'onemeta' ) ) {
			return;
		}
