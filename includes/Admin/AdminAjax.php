...
class AdminAjax {
	...
	public function handle_export(): void {
		...
		$result = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM %i WHERE group_key = %s', $table_name, $group_id ),
			ARRAY_A
		);
	...
	public function handle_toggle_status(): void {
		...
		$current = $wpdb->get_var(
			$wpdb->prepare( 'SELECT status FROM %i WHERE group_key = %s', $table_name, $group_id )
		);
	...
	public function handle_save_field_group(): void {
		...
		$group_type  = isset( $_POST['group_type'] ) ? sanitize_text_field( wp_unslash( $_POST['group_type'] ) ) : 'post';
		$post_type   = isset( $_POST['post_type'] ) ? sanitize_text_field( wp_unslash( $_POST['post_type'] ) ) : '';
		$position    = isset( $_POST['position'] ) ? sanitize_text_field( wp_unslash( $_POST['position'] ) ) : 'normal';
		...
		// Parse fields
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- recursively sanitized inside parse_fields() via sanitize_key()/sanitize_text_field()/sanitize_array_setting().
		$fields           = $this->parse_fields( wp_unslash( $_POST['fields'] ?? [] ) );
	...
	private function save_to_database( string $group_key, string $group_title, array $config ): void {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom plugin table; no core WP API exists for this query.
		$exists = $wpdb->get_var(
			$wpdb->prepare( 'SELECT id FROM %i WHERE group_key = %s', $table, $group_key )
		);
