...
class FieldGroupEditor {
	...
	public static function get_field_group_for_edit( string $group_id = '' ): array {
		...
		$result = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM %i WHERE group_key = %s', $table, $group_id ),
			ARRAY_A
		);
	...
	public static function get_field_group_by_key( string $group_key ): ?array {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom plugin table; no core WP API exists for this query.
		$result = $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM %i WHERE group_key = %s', $table, $group_key ),
			ARRAY_A
		);
	...
	public static function field_group_exists( string $group_key ): bool {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom plugin table; no core WP API exists for this query.
		$exists = $wpdb->get_var(
			$wpdb->prepare( 'SELECT COUNT(*) FROM %i WHERE group_key = %s', $table, $group_key )
		);
